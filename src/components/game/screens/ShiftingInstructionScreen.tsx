...
function ShiftingInstructionScreen({
  ...
  const shiftTexts = (level.params.shiftTexts as string[]) ?? ["Tap once", "Don't tap!"];
  const settleAfter = ((level.params.settleAfter as number) ?? 2.0) * 1000;
  const shiftSpeed = ((level.params.shiftSpeed as number) ?? 0.4) * 1000;

  // Cycle through instructions
  useEffect(() => {
    if (!isPlaying || settled) return;
    const interval = setInterval(() => {
      setCurrentTextIndex((i) => (i + 1) % shiftTexts.length);
    }, shiftSpeed);
    return () => clearInterval(interval);
  }, [isPlaying, settled, shiftTexts.length, shiftSpeed]);

  // Settle on final instruction after delay
  useEffect(() => {
    if (!isPlaying) return;
    setSettled(false);
    ...
    const timer = setTimeout(() => {
      // Final text is the real instruction (last in array)
      setCurrentTextIndex(shiftTexts.length - 1);
      setSettled(true);
    }, settleAfter);
  ...
  const displayedText = shiftTexts[currentTextIndex] ?? "";
      ...
      <View style={styles.instructionContainer}>
        {isFailed ? (
          ...
          <View style={styles.shiftArea}>
            {!settled && (
              <Text style={styles.shiftHint}>WAIT FOR IT...</Text>
            )}
            <Text style={[
              styles.shiftText,
              {
                color: settled ? COLORS.white : COLORS.warning,
            ...
            {settled && (
              <Text style={styles.settledHint}>NOW!</Text>
            )}
          </View>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  instructionContainer: { minHeight: 60, justifyContent: "center", alignItems: "center" },
  statusText: { fontSize: 22, fontFamily: FONTS.bold, letterSpacing: 2 },
  shiftArea: { alignItems: "center", gap: 4 },
  shiftHint: { fontSize: 9, fontFamily: FONTS.regular, letterSpacing: 3, color: COLORS.textMuted },
  shiftText: { fontSize: 22, fontFamily: FONTS.bold, letterSpacing: 2, textAlign: "center" },
  settledHint: { fontSize: 12, fontFamily: FONTS.bold, letterSpacing: 4, color: COLORS.accent },
...
export default memo(ShiftingInstructionScreen);