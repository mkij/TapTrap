...
function DualInstructionScreen({
  ...
  const fakeInstruction = (level.params.fakeInstruction as string) ?? "Don't tap!";
  const hintColor = (level.params.hintColor as string) ?? "green";
  const correctPosition = (level.params.correctPosition as string) ?? "top";

  const realColor = HINT_COLORS[hintColor] ?? COLORS.accent;
  const fakeColor = "rgba(255,255,255,0.35)";

  const topText = correctPosition === "top" ? level.instruction : fakeInstruction;
  const bottomText = correctPosition === "top" ? fakeInstruction : level.instruction;
  const topColor = correctPosition === "top" ? realColor : fakeColor;
  const bottomColor = correctPosition === "top" ? fakeColor : realColor;
    ...
    <View style={styles.playArea}>
      ...
      <View style={styles.instructionContainer}>
        {isFailed ? (
          ...
          <View style={styles.dualArea}>
            <Text style={[styles.hintText, { color: realColor }]}>
              Follow {hintColor.toUpperCase()}
            </Text>
            <View style={styles.dualRow}>
              <Text style={[styles.dualText, { color: topColor }]}>{topText}</Text>
              <View style={styles.dualDivider} />
              <Text style={[styles.dualText, { color: bottomColor }]}>{bottomText}</Text>
            </View>
          </View>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  instructionContainer: { minHeight: 70, justifyContent: "center", alignItems: "center" },
  statusText: { fontSize: 22, fontFamily: FONTS.bold, letterSpacing: 2 },
  dualArea: { alignItems: "center", gap: 6 },
  hintText: { fontSize: 10, fontFamily: FONTS.bold, letterSpacing: 4 },
  dualRow: { alignItems: "center", gap: 4 },
  dualText: { fontSize: 18, fontFamily: FONTS.bold, letterSpacing: 2, textAlign: "center" },
  dualDivider: { width: 40, height: 1, backgroundColor: "rgba(255,255,255,0.1)", marginVertical: 2 },
...
export default memo(DualInstructionScreen);