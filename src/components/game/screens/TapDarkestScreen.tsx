...
function TapDarkestScreen({
    ...
    const count = (level.params.circleCount as number) ?? 4;

    const circles = useMemo(() => {
        // Opacities from light to dark
        const opacities = [0.08, 0.15, 0.25, 0.45, 0.12, 0.35].slice(0, count);
        const darkest = Math.min(...opacities);
        const positions = [
            { x: -35, y: -35 }, { x: 35, y: -35 },
            { x: -35, y: 35 }, { x: 35, y: 35 },
            { x: 0, y: -45 }, { x: 0, y: 45 },
        ...
        return opacities.map((opacity, i) => ({
            opacity,
            isDarkest: opacity === darkest,
            x: shuffledPositions[i].x,
            y: shuffledPositions[i].y,
            size: 28, // same size, different brightness
        }));
    }, [level.id, count]);
    ...
    const handleCircleTap = (isDarkest: boolean) => {
        if (!isPlaying) return;
        if (isDarkest) {
            onTap();
        } else {
                    ...
                    {isFailed ? (
                        ...
                        <View style={styles.circlesArea}>
                            {circles.map((c, i) => (
                                <Pressable
                                    key={i}
                                    onPress={() => handleCircleTap(c.isDarkest)}
                                    style={[
                                        styles.targetCircle,
                                        {
                                            width: c.size * 2,
                                            ...
                                            borderColor: `rgba(255,255,255,${c.opacity + 0.05})`,
...
const styles = StyleSheet.create({
    ...
    targetCircle: { position: "absolute", borderWidth: 1 },
...
export default memo(TapDarkestScreen);