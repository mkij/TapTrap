...
function ShakeScreen({
  ...
  useEffect(() => {
    ...
    const subscription = Accelerometer.addListener(({ x, y, z }) => {
      ...
      if (total > SHAKE_THRESHOLD && !completedRef.current) {
      ...
      <View style={styles.ringContainer}>
        ...
        <Pressable onPress={handleTap}>
          <View style={[styles.inner, {
            borderColor: isFailed ? "rgba(255,51,85,0.25)" : isLevelComplete ? "rgba(0,255,136,0.25)" : "rgba(255,170,0,0.2)",
            backgroundColor: isFailed ? "rgba(255,26,26,0.07)" : isLevelComplete ? "rgba(0,255,136,0.07)" : "rgba(255,170,0,0.04)",
          }]}>
            {isFailed ? (
              ...
              <Animated.View style={[styles.shakeContent, { transform: [{ rotate: wobbleRotation }] }]}>
                <Text style={styles.shakeIcon}>📱</Text>
                <Text style={styles.shakeLabel}>SHAKE</Text>
              </Animated.View>
            )}
          </View>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  shakeContent: { alignItems: "center", gap: 8 },
  shakeIcon: { fontSize: 48 },
  shakeLabel: { fontSize: 14, fontFamily: FONTS.bold, letterSpacing: 6, color: COLORS.warning },
...
export default memo(ShakeScreen);