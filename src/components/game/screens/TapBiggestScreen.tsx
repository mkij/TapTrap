...
function TapBiggestScreen({
  ...
  // Generate circles with different sizes
  const circles = useMemo(() => {
    const sizes = [18, 24, 30, 38, 22, 28].slice(0, count);
  ...
  const handleCircleTap = (isBiggest: boolean) => {
    if (!isPlaying) return;
    if (isBiggest) {
      onTap(); // correct
    } else {
      onAction("hold_end"); // wrong target → fail
    }
  };
          ...
          {isFailed ? (
            ...
            <View style={styles.circlesArea}>
              {circles.map((c, i) => (
                <Pressable
                  key={i}
                  onPress={() => handleCircleTap(c.isBiggest)}
                  style={[
                    styles.targetCircle,
                    {
                      width: c.size * 2,
...
const styles = StyleSheet.create({
  ...
  targetCircle: { position: "absolute", backgroundColor: "rgba(255,255,255,0.12)", borderWidth: 1, borderColor: "rgba(255,255,255,0.2)" },
...
export default memo(TapBiggestScreen);