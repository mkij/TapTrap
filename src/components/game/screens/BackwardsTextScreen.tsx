...
function BackwardsTextScreen({
  ...
  // Reversed instruction is in level.instruction
  // Real instruction in params.realInstruction for reveal
  const reversedText = level.instruction;
  const realText = (level.params.realInstruction as string) ?? "";
  ...
  return (
    <View style={styles.playArea}>
      ...
      {/* Reversed instruction with mirror hint */}
      <View style={styles.instructionContainer}>
        {isFailed ? (
          <View style={styles.revealArea}>
            <Text style={[styles.statusText, { color: COLORS.danger }]}>FAILED</Text>
            <Text style={styles.revealText}>It said: {realText}</Text>
          </View>
        ) : isLevelComplete ? (
          <View style={styles.revealArea}>
            <Text style={[styles.statusText, { color: COLORS.accent }]}>NICE!</Text>
            <Text style={styles.revealText}>{realText}</Text>
          </View>
        ) : (
          <View style={styles.backwardsArea}>
            <Text style={styles.mirrorLabel}>↔ READ BACKWARDS</Text>
            <Text style={styles.backwardsText}>{reversedText}</Text>
          </View>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  backwardsArea: { alignItems: "center", gap: 4 },
  mirrorLabel: { fontSize: 9, fontFamily: FONTS.regular, letterSpacing: 3, color: COLORS.textMuted },
  backwardsText: { fontSize: 22, fontFamily: FONTS.bold, letterSpacing: 3, color: COLORS.warning },
...
export default memo(BackwardsTextScreen);