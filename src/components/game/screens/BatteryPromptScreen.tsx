...
function BatteryPromptScreen({
  ...
  // Read battery on mount
  useEffect(() => {
    Battery.getBatteryLevelAsync().then((level) => {
      setBatteryLevel(Math.round(level * 100));
    }).catch(() => {
      setBatteryLevel(50); // fallback
    });
  }, []);

  // Inject batteryTens into level params for validator
  useEffect(() => {
    if (batteryLevel !== null) {
      const tens = Math.floor(batteryLevel / 10);
      level.params.batteryTens = tens;
    }
  }, [batteryLevel, level]);
  ...
  const tens = batteryLevel !== null ? Math.floor(batteryLevel / 10) : "?";
      ...
      <View style={styles.ringContainer}>
        ...
        <Pressable onPress={handleTap}>
          <View style={[styles.inner, {
            borderColor: isFailed ? "rgba(255,51,85,0.25)" : isLevelComplete ? "rgba(0,255,136,0.25)" : "rgba(0,200,100,0.2)",
            backgroundColor: isFailed ? "rgba(255,26,26,0.07)" : isLevelComplete ? "rgba(0,255,136,0.07)" : "rgba(0,200,100,0.04)",
        ...
        {isFailed ? (
          <View style={styles.revealArea}>
            ...
            <Text style={styles.revealText}>
              Battery: {batteryLevel}% → tens digit = {tens}
            </Text>
          </View>
        ) : isLevelComplete ? (
          <View style={styles.revealArea}>
            ...
            <Text style={styles.revealText}>
              Battery: {batteryLevel}% → {tens} taps
            </Text>
          </View>
        ) : (
...
const styles = StyleSheet.create({
  ...
  tapContent: { alignItems: "center", gap: 6 },
  batteryIcon: { fontSize: 32 },
  tapCount: { fontSize: 60, fontFamily: FONTS.light, color: COLORS.white, lineHeight: 68 },
  instructionContainer: { minHeight: 40, justifyContent: "center", alignItems: "center" },
  revealArea: { alignItems: "center", gap: 4 },
  revealText: { fontSize: 11, fontFamily: FONTS.regular, letterSpacing: 1, color: COLORS.textMuted },
...
export default React.memo(BatteryPromptScreen);