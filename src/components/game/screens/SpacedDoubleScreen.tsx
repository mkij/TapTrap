...
const COOLDOWN_R = 106;
const COOLDOWN_CIRC = 2 * Math.PI * COOLDOWN_R;
...
function SpacedDoubleScreen({
  ...
  const minGapMs = (level.params.minGapMs as number) ?? 1500;
  ...
  // Cooldown timer after first tap
  useEffect(() => {
    if (!firstTapTime || !isPlaying) return;

    const interval = setInterval(() => {
      const elapsed = Date.now() - firstTapTime;
      const prog = Math.min(elapsed / minGapMs, 1);
      setCooldownProgress(prog);
      if (prog >= 1) {
  ...
  }, [firstTapTime, isPlaying, minGapMs]);
  ...
  const handleTap = () => {
    ...
    if (state.tapCount === 0) {
      // First tap — start cooldown
      setFirstTapTime(Date.now());
      onTap();
    } else if (state.tapCount === 1) {
      // Second tap — check cooldown
      if (cooldownReady) {
        onTap(); // validator will pass (count === 2)
      } else {
        onAction("hold_end"); // too early — fail
      }
    } else {
      onTap(); // over-tap — validator will fail
    }
  };
  ...
  const cooldownColor = cooldownReady ? COLORS.accent : COLORS.warning;
    ...
    <View style={styles.playArea}>
      ...
      <View style={styles.ringContainer}>
        ...
        <Svg width={RING_SIZE} height={RING_SIZE} style={styles.ring}>
          ...
          {/* Cooldown ring — inside main ring, spins opposite direction */}
          {isPlaying && firstTapTime && !cooldownReady && (
            <>
              <Circle cx={cx} cy={cy} r={COOLDOWN_R} fill="none"
                stroke="rgba(255,255,255,0.06)" strokeWidth={4} />
              <Circle cx={cx} cy={cy} r={COOLDOWN_R} fill="none"
                stroke={cooldownColor} strokeWidth={4}
                strokeDasharray={`${COOLDOWN_CIRC * cooldownProgress} ${COOLDOWN_CIRC}`}
          ...
          <View style={[styles.inner, {
            ...
            {isFailed ? (
              ...
              <View style={styles.tapContent}>
                {/* Cooldown ring after first tap */}
                {firstTapTime && cooldownReady && (
...
const styles = StyleSheet.create({
  ...
  waitLabel: { fontSize: 10, fontFamily: FONTS.bold, letterSpacing: 4, color: COLORS.warning, marginTop: 4 },
  readyLabel: { fontSize: 14, fontFamily: FONTS.bold, letterSpacing: 6, color: COLORS.accent },
...
export default memo(SpacedDoubleScreen);