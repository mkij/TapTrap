...
function RecallDistantScreen({
  ...
  const hasMemory = targetNumber !== null;
  ...
  const stepsLabel = stepsBack === 1 ? "last" : stepsBack === 2 ? "2nd last" : `${stepsBack}th last`;
      ...
      <View style={styles.ringContainer}>
        ...
        <Pressable onPress={handleTap}>
          <View style={[styles.inner, {
            borderColor: isFailed ? "rgba(255,51,85,0.25)" : isLevelComplete ? "rgba(0,255,136,0.25)" : "rgba(138,43,226,0.2)",
            backgroundColor: isFailed ? "rgba(255,26,26,0.07)" : isLevelComplete ? "rgba(0,255,136,0.07)" : "rgba(138,43,226,0.04)",
            ...
            {isFailed ? (
              ...
              <View style={styles.tapContent}>
                <Text style={styles.memoryLabel}>RECALL</Text>
                <Text style={styles.tapCount}>{state.tapCount}</Text>
                {!hasMemory && (
                  <Text style={styles.noMemory}>no history</Text>
                )}
              </View>
            )}
          </View>
      ...
      <View style={styles.instructionContainer}>
        {isFailed ? (
          <View style={styles.revealArea}>
            ...
            <Text style={styles.revealText}>The {stepsLabel} number was {targetNumber ?? "?"}</Text>
          </View>
        ) : isLevelComplete ? (
          <View style={styles.revealArea}>
            <Text style={[styles.statusText, { color: COLORS.accent }]}>NICE!</Text>
            <Text style={styles.revealText}>Correct! It was {targetNumber}</Text>
          </View>
        ) : (
          <View style={styles.recallArea}>
            <Text style={styles.recallInstruction}>
              Tap the {stepsLabel} number you memorized
            </Text>
          </View>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  memoryLabel: { fontSize: 10, color: "rgba(138,43,226,0.6)", letterSpacing: 4, fontFamily: FONTS.bold },
  tapCount: { fontSize: 60, fontFamily: FONTS.light, color: COLORS.white, lineHeight: 68, marginTop: 2 },
  noMemory: { fontSize: 9, color: COLORS.textMuted, letterSpacing: 2, fontFamily: FONTS.regular, marginTop: 4 },
  ...
  recallArea: { alignItems: "center" },
  recallInstruction: { fontSize: 14, fontFamily: FONTS.bold, letterSpacing: 1, color: COLORS.white, textAlign: "center" },
...
export default memo(RecallDistantScreen);