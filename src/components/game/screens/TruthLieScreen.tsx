...
function TruthLieScreen({
  ...
  const isLie = (level.params.isLie as boolean) ?? false;
  const displayedInstruction = (level.params.displayedInstruction as string) ?? level.instruction;

  const labelPulse = useRef(new Animated.Value(1)).current;

  // Pulse the TRUTH/LIE label
  useEffect(() => {
    if (!isPlaying) return;
    const anim = Animated.loop(
      Animated.sequence([
        Animated.timing(labelPulse, { toValue: 0.5, duration: 800, useNativeDriver: true }),
        Animated.timing(labelPulse, { toValue: 1, duration: 800, useNativeDriver: true }),
      ])
    );
    anim.start();
    return () => anim.stop();
  ...
  const labelColor = isLie ? COLORS.danger : COLORS.accent;
  const labelText = isLie ? "LIE" : "TRUTH";
  ...
  return (
    <View style={styles.playArea}>
      ...
      <View style={styles.ringContainer}>
        ...
        <Pressable onPress={handleTap}>
          <View style={[styles.inner, {
            borderColor: isFailed ? "rgba(255,51,85,0.25)" : isLevelComplete ? "rgba(0,255,136,0.25)" : `${labelColor}30`,
            backgroundColor: isFailed ? "rgba(255,26,26,0.07)" : isLevelComplete ? "rgba(0,255,136,0.07)" : `${labelColor}06`,
            ...
            {isFailed ? (
              ...
              <View style={styles.truthLieContent}>
                <Animated.Text style={[styles.label, { color: labelColor, opacity: labelPulse }]}>
                  {labelText}
                </Animated.Text>
      ...
      {/* Instruction with truth/lie context */}
      <View style={styles.instructionContainer}>
        {isFailed ? (
          <View style={styles.revealArea}>
            <Text style={[styles.statusText, { color: COLORS.danger }]}>FAILED</Text>
            <Text style={styles.revealText}>
              {isLie ? `It was a lie! Do opposite of "${displayedInstruction}"` : `It was truth! Follow "${displayedInstruction}"`}
            </Text>
          </View>
        ) : isLevelComplete ? (
...
const styles = StyleSheet.create({
  ...
  truthLieContent: { alignItems: "center", gap: 4 },
  label: { fontSize: 16, fontFamily: FONTS.bold, letterSpacing: 6 },
  tapCount: { fontSize: 48, fontFamily: FONTS.light, color: COLORS.white, lineHeight: 54, marginTop: 2 },
  instructionContainer: { minHeight: 50, justifyContent: "center", alignItems: "center" },
  instructionText: { fontSize: 22, fontFamily: FONTS.bold, letterSpacing: 2, color: COLORS.white, textAlign: "center" },
  statusText: { fontSize: 22, fontFamily: FONTS.bold, letterSpacing: 2 },
  revealArea: { alignItems: "center", gap: 4 },
  revealText: { fontSize: 10, fontFamily: FONTS.regular, letterSpacing: 1, color: COLORS.textMuted, textAlign: "center", paddingHorizontal: 20 },
...
export default memo(TruthLieScreen);